...
rankhospital <- function(state, restriccion, num = "best") {
  ...
  names(subOutcome) = c("Estado","Hospital","ZIP","Rate")
  
  ## --- Solo recuperar datos del estado/provincia seleccionado
  outcome <- subset(outcome,Estado == state)
  
  textosAValidar <- c("best","worst")
  num <- stri_trans_general(num,id="lower")
  
  
  
  if(is.numeric(num)){
    num <- as.numeric(num)
    tamanio <- nrow(outcome)
    if (tamanio > num)
    {
      stop('El tamaño num es superior a los datos del estado')
    }
  }else if(!(num %in% textosAValidar)){
    stop('num puede tomar el valor de best o worst')
  }
  
   #texto a número

  outcome$Data  <- as.numeric(subOutcome$Rate)
  
   ## --- Eliminar NA
  
  outcome <- subOutcome %>% filter(!is.na(Rate))
